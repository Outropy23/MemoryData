...
class Swarm64MemoryBackend:
    ...
    DDL_COLUMNAR_INDEX = """
    -- Swarm64 columnar foreign table for accelerated range scans
    ...
    CREATE TABLE IF NOT EXISTS worm_audit_log (
        ...
        chain_hash      TEXT NOT NULL,   -- Merkle chain link
        timestamp_utc   TIMESTAMPTZ DEFAULT now()
    );
    -- Immutability trigger: block DELETE and UPDATE on sealed rows
    ...
    def _init_schema(self) -> None:
        with self.conn.cursor() as cur:
            ...
            # Columnar acceleration (optional)
            if self.cfg.s64da_enable_columnar:
                try:
    ...
    def insert_memory(
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute(
                """
                INSERT INTO worm_audit_log (memory_id, operation, actor, payload_hash, chain_hash)
    ...
    def vector_search(
        ...
        """
        filters = ["entropy_score < %s", "worm_sealed = TRUE OR worm_sealed = FALSE"]
        ...
        sql = f"""
            ...
            LIMIT %s;
        """
        params.insert(-1, query_embedding.tolist())  # for ORDER BY
    ...
    def seal_memory(self, memory_id: str, actor: str = "gotham_v3_supervisor") -> None:
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                "UPDATE agent_memory SET worm_sealed = TRUE, updated_at = now() WHERE memory_id = %s;",
                (memory_id,)
            )
