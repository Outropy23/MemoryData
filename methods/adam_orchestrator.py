...
class AdamOrchestrator:
    ...
    def _extraction_pipeline(self, obs: str, agent_id: str, embedding_fn, idx: int) -> Dict:
        t0 = time.monotonic()
        # Step 1: Entity extraction via LLM sub-agent
        entities = self._run_entity_extractor(obs)
        # Step 2: Temporal tagging
        temporal_tags = self._run_temporal_tagger(obs, entities)
        # Step 3: Entropy guard
    ...
    def _run_temporal_tagger(self, text: str, entities: List[str]) -> Dict:
        return {"raw_text_length": len(text), "entities_count": len(entities), "tagged_at": datetime.now(timezone.utc).isoformat()}
    ...
    def _compute_entropy(self, text: str) -> float:
        ...
        return min(H / 8.0, 1.0)  # normalize by max entropy of 8 bits
    ...
    def maintain_parallel(
        ...
        futures = {
            ...
            "forget": self.maintenance_pool.submit(self._run_forgetting_agent, agent_id, decay_threshold),
        }
        results = {}
        ...
        if seal_finalized:
            sealed = self._run_worm_auditor(agent_id, results)
            results["worm_sealed_count"] = sealed

        self.gotham.emit_maintenance_report(agent_id, results)
        return results

    def _run_conflict_detector(self, agent_id: str) -> Dict:
        return {"conflicts_found": 0, "agent_id": agent_id, "checked_at": datetime.now(timezone.utc).isoformat()}

    def _run_consolidator(self, agent_id: str) -> Dict:
        return {"merged": 0, "updated": 0, "agent_id": agent_id}

    def _run_forgetting_agent(self, agent_id: str, threshold: float) -> Dict:
    ...
    def _run_worm_auditor(self, agent_id: str, maintenance_report: Dict) -> int:
        return 0  # override with backend.seal_memory() calls in production
