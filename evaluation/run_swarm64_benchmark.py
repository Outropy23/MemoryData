...
Wires evaluation/ scripts against the new PostgreSQL backend.
...
def run_workload(workload_id: str, backend: Swarm64MemoryBackend, gotham: GothamV3Supervisor, output_dir: str) -> dict:
    print(f"  Running workload: {workload_id} ...", flush=True)
    import numpy as np
    metrics = {"workload": workload_id, "insert_latencies_ms": [], "retrieval_latencies_ms": []}
    ...
    for q in range(n_queries):
        ...
        t0 = time.monotonic()
        results = backend.vector_search(qemb, top_k=10, module_filter="extraction")
    ...
    # WORM-seal all benchmark inserts
...
def main():
    ...
    cfg = Swarm64Config(
        ...
        database=args.pg_db, user=args.pg_user, password=args.pg_password,
        s64da_enable_columnar=True, s64da_parallel_workers=64,
    )
    backend = Swarm64MemoryBackend(cfg)
    backend.connect()
    gotham = GothamV3Supervisor(log_dir=os.path.join(args.output_dir, "gotham_v3_logs"))

    all_metrics = []
    print(f"Running {len(args.workloads)} workloads against Swarm64 backend ...")
    for wid in args.workloads:
        m = run_workload(wid, backend, gotham, args.output_dir)
    ...
    manifest = gotham.export_compliance_manifest()
