...
WORKLOAD_IDS = ["single_session", "multi_session", "knowledge_update", "long_horizon", "adversarial"]
WORKLOAD_LABELS = ["Single\nSession", "Multi\nSession", "Knowledge\nUpdate", "Long\nHorizon", "Adversarial"]
...
def plot_figure2(data: dict, output: str) -> None:
    ...
    fig.suptitle(
        ...
        "Workloads: MemoryData Benchmark Suite (arXiv:2606.24775) · ItalyWorld R&D / Gotham v3",
    ...
    insert_data = [data[wid]["insert_latencies_ms"] for wid in WORKLOAD_IDS if wid in data]
    ...
    ax_insert.set_xticklabels([WORKLOAD_LABELS[i] for i, wid in enumerate(WORKLOAD_IDS) if wid in data])
    ...
    retrieval_data = [data[wid]["retrieval_latencies_ms"] for wid in WORKLOAD_IDS if wid in data]
    ...
    ax_retrieval.set_xticklabels([WORKLOAD_LABELS[i] for i, wid in enumerate(WORKLOAD_IDS) if wid in data])
    ...
    # Panel C: P50 vs P99 comparison bar
    ...
    ax_bar.set_xticks(x)
    ax_bar.set_xticklabels([WORKLOAD_LABELS[i] for i, _ in enumerate(workloads_present)], fontsize=8)
    ax_bar.set_ylabel("ms")
    ax_bar.set_title("(C) Insert P50 vs P99", fontweight="bold")
    ax_bar.legend(fontsize=9)

    # Panel D: P50 retrieval bar
    ...
    ax_rbar.set_xticklabels([WORKLOAD_LABELS[i] for i, _ in enumerate(workloads_present)], fontsize=8)
